...
status_t buildResources(Bundle* bundle, const sp<AaptAssets>& assets, sp<ApkBuilder>& builder)
{
    ...
    ssize_t packageId = -1;
    if (bundle->getPackageId()) {
        packageId = atoi(bundle->getPackageId());
        if (!(packageId > 0x01 && packageId < 0x7F)) {
            fprintf(stderr, "Error: --package-id should be 2-126 got(%d)", (int) packageId);
            return UNKNOWN_ERROR;
        }
    }

    ResourceTable table(bundle, String16(assets->getPackage()), packageType, packageId);
